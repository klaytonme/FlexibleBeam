...
void AccelStepper::setBound(int bound) { _bound = bound; }
...
boolean AccelStepper::runSpeedBounded() {
	if (_direction == DIRECTION_CW && _currentPos >= _bound) return false;
	if (_direction == DIRECTION_CCW && _currentPos <= -_bound) return false;
	runSpeed();
}
...
void AccelStepper::stop() {
	if (_speed != 0.0) {
		...
		if (_speed > 0) move(stepsToStop);
		else move(-stepsToStop);
	}
}
